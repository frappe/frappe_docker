...
### Статический (не-Frappe) поддомен

Другой случай, соседний с «ещё один клиент» выше, но не тот же самый: домен
всё ещё под wildcard DNS-записью и wildcard-сертификатом, но это не сайт
Frappe. `bench new-site` тут ни при чём, и общий `SITES_RULE` (`HostRegexp`)
не должен решать, куда идёт трафик — каталога в `sites/` для такого имени нет
и не будет, а значит nginx фронтенда ответит ошибкой, если запрос всё-таки
туда попадёт.

Первый пример — `distrib.habibi-erp.com`, статический SPA
(`habibi-distributor`): отдельный контейнер nginx, без БД, без bench. Оверрайд
`habibi/overrides/compose.distrib.yaml`. Что в нём сделано и почему (детали —
в заголовке самого файла):

- отдельный Traefik-роутер с явным `` Host(`distrib.${BASE_DOMAIN}`) ``, а не
  участие в общем `SITES_RULE`;
- явный `priority=100` у этого роутера — без него `HostRegexp` фронтенда всё
  равно победил бы: приоритет по умолчанию Traefik считает по длине строки
  правила, а `HostRegexp` длиннее простого `Host(...)`;
- `tls.domains` не переопределяется — сертификат `*.habibi-erp.com` общий,
  Traefik подбирает его по SNI независимо от того, какой роутер его заказал.

Бринг-ап на сервере — правка `.env` и один сервис, без рестарта остального
стека:

```bash
# .env:
#   COMPOSE_FILE=...:habibi/overrides/compose.distrib.yaml
#   DISTRIB_IMAGE=ghcr.io/dhi-partners/habibi-distributor   # опционально, это и default
#   DISTRIB_TAG=v1.0.0                                       # версионный тег из CI

docker compose up -d distrib
```

Это шаблон для любого будущего статического поддомена, не только `distrib`:
скопировать файл, поменять имя сервиса, образ/теги в `.env` и `Host(...)` в
правиле роутера.
